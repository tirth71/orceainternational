...
export function DivisionLayout({
  ...
  return (
    <>
      <PageHero eyebrow={d.short} title={heroTitle} description={heroDescription}>
        <div className="flex flex-wrap gap-3">
          ...
          <Link to="/divisions" className="inline-flex items-center rounded-full border border-white/20 bg-white/5 px-6 py-3.5 text-sm font-semibold text-primary-foreground hover:bg-white/10">
            All Divisions
          </Link>
        </div>
      </PageHero>
...
export function ServicesGrid({ title, eyebrow, items }: { title: string; eyebrow: string; items: { title: string; desc: string }[] }) {
  return (
    <Section>
      <SectionHeading eyebrow={eyebrow} title={title} center />
      <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {items.map((it) => (
          <div key={it.title} className="rounded-2xl border border-border bg-card p-6 shadow-soft transition hover:-translate-y-1 hover:shadow-elegant">
...
export function ProcessTimeline({ steps }: { steps: { title: string; desc: string }[] }) {
  return (
    <Section surface>
      <SectionHeading eyebrow="How it works" title="Our process, simplified." center />
